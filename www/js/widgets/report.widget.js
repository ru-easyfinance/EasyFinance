    ...
    function ShowDetailedIncome(data, currencyId){
        ...
        for (var key in data[0]){
            ...
            if (data[0][key].account_name != null) {
                tr += "<tr>"
                    + '<td>&nbsp;</td>'
                    + '<td><span>'+data[0][key].date+'</span></td>'
                    + '<td class="light"><span>'+data[0][key].account_name+'</span></td>'
                    + '<td class="' + (data[0][key].money >=0 ? 'sumGreen' : 'sumRed')+'"><span>'+formatCurrency(data[0][key].money)+'</span></td>'
                    + '</tr>';
            }
        }
    ...
    function ShowDetailedWaste(data, currencyId){
        ...
        for (var key in data[0]){
            ...
            if (data[0][key].cat_name != null && data[0][key].account_name != null){
                tr += "<tr>" +
                    '<td>&nbsp;</td>' +
                    '<td class="repdate"><span>'+data[0][key].date+'</span></td>' +
                    '<td class="repname"><span>'+data[0][key].account_name+'</span></td>' +
                    '<td class="' + (data[0][key].money >= 0 ? 'sumGreen' : 'sumRed')+'"><span>'+formatCurrency(data[0][key].money)+'</span></td>' +
                    '</tr>';
                }
            }
