...
if ($this->Context->WarningCollector->Count() > 0) {
	...
	$CommentList .= '<div class="ContentInfo Top">
		<h1>';
			if ($this->Context->Configuration['USE_CATEGORIES']) $CommentList .= '<a href="'.GetUrl($this->Context->Configuration, 'index.php', '', 'CategoryID', $this->Discussion->CategoryID).'">'.$this->Discussion->Category.'</a> &raquo; &nbsp;';
	...
	while ($Row = $this->Context->Database->GetRow($this->CommentData)) {
		...
		$CommentList .= '<li id="Comment_'.$Comment->CommentID.'"'.($CommentClass == ''?'':' class="'.$CommentClass.'"').'>
			...
			$CommentList .= '<div class="CommentBody" id="CommentBody_'.$Comment->CommentID.'"><div class="Bubble"><div><div><div><div><div>';
