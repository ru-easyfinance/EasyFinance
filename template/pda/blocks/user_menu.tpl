<table class="topmenu wide" cellpadding="0" cellspacing="0"><tbody><tr>
	<td style="background:#ffffff url(/img/pda/menuLine.gif) no-repeat right">
		<a href="/operation/add/waste"><img src="/img/pda/menuOperations.gif" alt="" /><br>добавить</a>
	</td>
	<td style="background:#ffffff url(/img/pda/menuLine.gif) no-repeat right">
		<a href="/operation/"><img src="/img/pda/menuOperations.gif" alt="" /><br>журнал</a>
	</td>
	<td style="background:#ffffff url(/img/pda/menuLine.gif) no-repeat right">
		<a href="/info"><img src="/img/pda/menuAccounts.gif" alt="" /><br>счета</a>
	</td>
	<td>
		<a href="/category"><img src="/img/pda/menuCategories.gif" alt="" /><br>категории</a>
	</td>
</tbody></tr></table>
