...
<div class="b-custom-input{if !$selectbox} no-selectbox{/if}{if $class} {$class}{/if}{if $disabled} b-disabled{/if}">
    <div class="b-custom-input-border">
        ...
        <input class="b-custom-input-field"{if $id} id="{$id}"{/if}{if $name} name="{$name}"{/if} type="{if $type}{$type}{else}text{/if}"{if $disabled} disabled{/if} value="{$value}"{if $maxlength} maxlength={$maxlength}{/if} />
        {if $selectbox}
