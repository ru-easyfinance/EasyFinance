...
{
    /**
     * Добавляет assets, вырезано упоминание о sfJqueryReloadedPlugin
     *
     * @param sfEvent $event
     */
    ...
    {
        $jquery_include_tag = '<script type="text/javascript" src="'
            . sfAdminDash::getProperty('web_dir')
            . '/js/'.sfAdminDash::getProperty('jquery_filename').'"></script>';
        ...
        if (false !== ($pos = strpos($content, $jquery_include_tag))) {
            $content = substr($content, 0, $pos + strlen($jquery_include_tag))
                . $jquery_no_conflict_tag
                . substr($content, $pos + strlen($jquery_include_tag));
        }
