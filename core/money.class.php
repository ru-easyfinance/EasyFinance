...
class Money  
{
	...
	var $current_date   = 0;
	var $account_money  = 0;
	var $user_money     = Array();
	var $total_sum		= 0;
	
	function Money(&$db, &$user)
	{
	...
	function load()
	{
		...
		$this->account_money = $_SESSION['account_money'];
		$this->total_sum = $_SESSION['total_sum'];
	}
	...
	function selectMoney($id)
	{
		if (!empty($_GET['order']))
		{
			if (html($_GET['order']) == "today")
			{
	...
	function getActiveAccount()
	{
		...
		$arr = array("id"=>$this->user_money[$i]['bill_id'], "name"=>$this->user_money[$i]['bill_name'], "money"=>$this->user_money[$i]['money'], "drain"=>$this->user_money[$i]['drain'], "currency"=>$this->user_money[$i]['bill_currency'], "cur_name"=>$this->user_money[$i]['cur_name']);		
		return $arr;
	}
	...
	{
		$sql = "
				SELECT m.`id`, m.`user_id`, m.`money`, m.`tr_id`,m.`transfer`, DATE_FORMAT(m.date,'%d.%m.%Y') as date, 
	 ...
	 * @param integer $drain Доход или расход. Устаревшее, но на всякий случай указывать надо
	 * @param string $comment Комментарий транзакции
	 * @param integer $bill_id Код счета
	 * @param string $impID Код импорта. Опционелен
  ...
  public function getProfit($rpd,$category) {
	...
	$dateTo = $year.".".$month.".".$day;
  	...
  	else
  	{
		...
		// Еще раз пробегаемся по массиву, чтобы посчитать долю каждого элемента
