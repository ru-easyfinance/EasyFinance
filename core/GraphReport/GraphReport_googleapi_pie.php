<?php
/**
 * Класс построителя Pie chart на основе Google API
 *
 ...
 *
 */ 
class GraphReport_googleapi_pie extends GraphReport implements iGraphReport  {
   ...
   */
	public function build($data) {
		...
		/**$keys = urlencode(join('|',array_keys($data)));
		$vals =  join(',',$vals);

		$url = "http://chart.apis.google.com/chart?cht=p3&chd=t:$vals&chs=800x300&chtt=My+chart&chl=$keys";
		//pre($url);
...
$piChart->valueLabels = array("тест", "тест1", "third","fourth");
$piChart->dataColors = array("ff3344", "11ff11", "22aacc", "3333aa");
$piChart->set3D(true);

$img = file_get_contents($piChart->getUrl());
