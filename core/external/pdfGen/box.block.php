<?php
// $Header: /cvsroot/html2ps/box.block.php,v 1.54 2006/12/18 19:44:21 Konstantin Exp $
...
class BlockBox extends GenericContainerBox {
  ...
  function show_fixed(&$driver) {
    ...
    if ($position == POSITION_FIXED) {
      ...
      if (!$top->isAuto()) {
        if ($top->isPercentage()) {
          $vertical_offset = $driver->getPageHeight() / 100 * $top->getPercentage();
        } else {
        ...
        if ($bottom->isPercentage()) {
          $vertical_offset = $driver->getPageHeight() * (100 - $bottom->getPercentage())/100 - $this->get_height();
        } else {
          $vertical_offset = $driver->getPageHeight() - $bottom->getPoints() - $this->get_height();
        };
